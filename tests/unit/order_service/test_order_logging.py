...
@pytest.fixture
def service_logger(log_output: StringIO, order_service_modules) -> logging.Logger:
    setup_logging("order-service", "debug")
    handler = logging.StreamHandler(log_output)
    handler.setFormatter(JsonFormatter(service_name="order-service"))
    logger = logging.getLogger("tests.order_service.logging")
...
def test_log_level_filters_output(log_output: StringIO) -> None:
    setup_logging("order-service", "warning")
...
def test_logger_name_preserved(service_logger: logging.Logger, log_output: StringIO) -> None:
    ...
    assert payload["logger"] == "tests.order_service.logging"
...
def test_uvicorn_access_log_parses_http_fields(log_output: StringIO) -> None:
    ...
    record = logging.LogRecord(
        ...
        lineno=1,
        msg='%s - "%s %s HTTP/%s" %d',
        args=("127.0.0.1:12345", "GET", "/orders", "1.1", 200),
        exc_info=None,
    )
...
def test_build_log_config_includes_uvicorn_loggers() -> None:
    config = build_log_config("order-service", "info")
