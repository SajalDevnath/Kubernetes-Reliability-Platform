...
@pytest.fixture
def metrics_client(order_service_modules) -> Generator[TestClient, None, None]:
...
def test_metrics_handler_uses_route_template_not_raw_id(metrics_client: TestClient) -> None:
    metrics_client.get("/orders/42")
    response = metrics_client.get("/metrics")

    body = response.text
    assert 'handler="/orders/{order_id}"' in body
    assert 'handler="/orders/42"' not in body
